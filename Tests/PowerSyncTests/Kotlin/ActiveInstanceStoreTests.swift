...
struct MultipleInstanceTest {
    @Test func warnsAboutMultipleInstances() async throws {
        ...
        let a = PowerSyncDatabaseImpl(identifier: "id", logger: logger, pool: pool, customHttpSession: nil as URLSession?, schema: schema)
        try #require(logWriter.getLogs().isEmpty)

        let b = PowerSyncDatabaseImpl(identifier: "id", logger: logger, pool: pool, customHttpSession: nil as URLSession?, schema: schema)
    ...
    @Test func doesNotWarnForClosedInstances() async throws {
        ...
        do {
            let _ = PowerSyncDatabaseImpl(identifier: "id2", logger: logger, pool: pool, customHttpSession: nil as URLSession?, schema: schema)
        }

        let b = PowerSyncDatabaseImpl(identifier: "id2", logger: logger, pool: pool, customHttpSession: nil as URLSession?, schema: schema)
        try #require(logWriter.getLogs().isEmpty)
        let _ = consume b
    }
}
